...
def floor_plan_from_scene(scene,
    ...
    floor, tr_floor = get_floor_plan(scene, [
...
def scene_from_args(args):
    ...
    scene.camera_matrix = Matrix44.orthogonal_projection(left=-args.room_side,
...
def make_network_input(current_boxes, indices):
    ...
    return dict(class_labels=_prepare(current_boxes["class_labels"][indices]),
...
def render_to_folder(args,
    ...
    bbox_params_t = torch.cat([
        boxes["class_labels"], boxes["translations"], boxes["sizes"],
        boxes["angles"]
    ],
                              dim=-1).cpu()
    ...
    render_simple_3dviz(renderables + floor_plan,
...
def render_scene_from_bbox_params(args, bbox_params, dataset, objects_dataset,
                                  classes, floor_plan, tr_floor, scene,
                                  path_to_image, path_to_objs):
    boxes = dataset.post_process(bbox_params)
    ...
    render_simple_3dviz(renderables,
                        behaviours=behaviours,
                        size=args.window_size,
                        camera_position=args.camera_position,
                        camera_target=args.camera_target,
